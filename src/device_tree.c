...
int
fdt_next_property(const void *blob, uint32_t offset,
                  struct fdt_property *prop)
{
    ...
    int index = 0;
    if (be32_to_cpu(ptr[index++]) != FDT_TOKEN_PROPERTY)
        return 0;

    uint32_t size = be32_to_cpu(ptr[index++]);
    uint32_t name_offset = be32_to_cpu(ptr[index++]);
    name_offset += be32_to_cpu(header->strings_offset);
...
int fdt_node_name(const void *blob, uint32_t offset, const char **name)
{
    ...
    if (be32_to_cpu(*(uint32_t *) ptr) != FDT_TOKEN_BEGIN_NODE)
        return 0;
...
static int print_flat_node(const void *blob, uint32_t start_offset,
                           int depth)
{
...
static struct device_tree_node *alloc_node(void)
{
    ...
    list_init(&buf->children);
    list_init(&buf->properties);
    return buf;
}
...
static int
fdt_unflatten_node(const void *blob, uint32_t start_offset,
                   struct device_tree_node **new_node)
{
    ...
    *new_node = node;
    ...
    while ((size = fdt_next_property(blob, offset, &fprop))) {
        ...
        list_insert_befor(&node->properties, &prop->list_node);
    ...
    while ((size = fdt_unflatten_node(blob, offset, &child))) {
        list_insert_befor(&node->children, &child->list_node);
...
static int
fdt_unflatten_map_entry(const void *blob, uint32_t offset,
                        struct device_tree_reserve_map_entry **new)
{
    const uint64_t *ptr = (const uint64_t *) (((uint8_t *) blob) + offset);
    const uint64_t start = be64_to_cpu(ptr[0]);
    const uint64_t size = be64_to_cpu(ptr[1]);
...
struct device_tree *fdt_unflatten(const void *blob)
{
    ...
    tree->header = header;
    list_init(&tree->reserve_map);

    uint32_t struct_offset = be32_to_cpu(header->structure_offset);
    uint32_t strings_offset = be32_to_cpu(header->strings_offset);
    uint32_t reserve_offset = be32_to_cpu(header->reserve_map_offset);
    ...
    while ((size = fdt_unflatten_map_entry(blob, offset, &entry))) {
        list_insert_befor(&tree->reserve_map, &entry->list_node);
        offset += size;
    }
...
static void
dt_flat_prop_size(struct device_tree_property *prop,
...
static void
dt_flat_node_size(struct device_tree_node *node,
                  uint32_t * struct_size, uint32_t * strings_size)
{
    // Starting token.
    *struct_size += sizeof(uint32_t);
    ...
    *struct_size += ALIGN_UP(strlen(node->name) + 1, sizeof(uint32_t));
    ...
    list_for_each(prop, node->properties, struct device_tree_property,
                  list_node) dt_flat_prop_size(prop, struct_size,
                                               strings_size);

    list_for_each(child, node->children, struct device_tree_node,
                  list_node)
...
uint32_t dt_flat_size(const struct device_tree *tree)
{
    ...
    list_for_each(entry, tree->reserve_map,
                  struct device_tree_reserve_map_entry, list_node) size +=
        sizeof(uint64_t) * 2;
...
static void
dt_flatten_map_entry(struct device_tree_reserve_map_entry *entry,
                     void **map_start)
{
    ((uint64_t *) * map_start)[0] = cpu_to_be64(entry->start);
    ((uint64_t *) * map_start)[1] = cpu_to_be64(entry->size);
    *map_start = ((uint8_t *) * map_start) + sizeof(uint64_t) * 2;
}

static void
dt_flatten_prop(struct device_tree_property *prop,
                ...
                void **strings_start)
{
    ...
    *((uint32_t *) dstruct) = cpu_to_be32(FDT_TOKEN_PROPERTY);
    dstruct += sizeof(uint32_t);

    *((uint32_t *) dstruct) = cpu_to_be32(prop->prop.size);
    dstruct += sizeof(uint32_t);

    uint32_t name_offset = (uintptr_t) dstrings - (uintptr_t) strings_base;
    *((uint32_t *) dstruct) = cpu_to_be32(name_offset);
    dstruct += sizeof(uint32_t);
...
static void
dt_flatten_node(const struct device_tree_node *node,
                void **struct_start, void *strings_base,
                void **strings_start)
{
    uint8_t *dstruct = (uint8_t *) * struct_start;
    ...
    *((uint32_t *) dstruct) = cpu_to_be32(FDT_TOKEN_BEGIN_NODE);
    dstruct += sizeof(uint32_t);
    ...
    list_for_each(prop, node->properties, struct device_tree_property,
                  list_node) dt_flatten_prop(prop, (void **) &dstruct,
                                             strings_base,
                                             (void **) &dstrings);
    ...
    *((uint32_t *) dstruct) = cpu_to_be32(FDT_TOKEN_END_NODE);
    dstruct += sizeof(uint32_t);
...
void dt_flatten(const struct device_tree *tree, void *start_dest)
{
    ...
    list_for_each(entry, tree->reserve_map,
                  ...
                  list_node) dt_flatten_map_entry(entry, (void **) &dest);
    ...
    header->structure_offset = cpu_to_be32(dest - (uint8_t *) start_dest);
    header->structure_size = cpu_to_be32(struct_size);
    dest += struct_size;

    *((uint32_t *) dest) = cpu_to_be32(FDT_TOKEN_END);
    dest += sizeof(uint32_t);

    uint8_t *strings_start = dest;
    header->strings_offset = cpu_to_be32(dest - (uint8_t *) start_dest);
    header->strings_size = cpu_to_be32(strings_size);
    dest += strings_size;

    dt_flatten_node(tree->root, (void **) &struct_start, strings_start,
                    (void **) &strings_start);

    header->totalsize = cpu_to_be32(dest - (uint8_t *) start_dest);
}
...
static void print_node(const struct device_tree_node *node, int depth)
{
    ...
    list_for_each(prop, node->properties, struct device_tree_property,
                  list_node) print_property(&prop->prop, depth + 1);

    list_for_each(child, node->children, struct device_tree_node,
                  list_node) print_node(child, depth + 1);
}
 ...
 */
void
dt_read_cell_props(const struct device_tree_node *node, uint32_t * addrcp,
                   uint32_t * sizecp)
{
    list_for_each(prop, node->properties, struct device_tree_property,
                  list_node) {
        if (addrcp && !strcmp("#address-cells", prop->prop.name))
            *addrcp = be32_to_cpu(*(uint32_t *) prop->prop.data);
        if (sizecp && !strcmp("#size-cells", prop->prop.name))
            *sizecp = be32_to_cpu(*(uint32_t *) prop->prop.data);
    }
}
...
struct device_tree_node *dt_find_node(struct device_tree_node *parent,
                                      const char **path, uint32_t * addrcp,
                                      uint32_t * sizecp, int create)
{
    struct device_tree_node *found = NULL;
    ...
    list_for_each(node, parent->children, struct device_tree_node,
                  list_node) {
    ...
    if (!found) {
        ...
        list_insert_befor(&parent->children, &found->list_node);
    }
 ...
 */
struct device_tree_node *dt_find_node_by_path(struct device_tree_node
                                              ...
                                              int create)
{
    ...
    if (!next_slash) {
        path_array[i] = NULL;
        node = dt_find_node(parent, path_array, addrcp, sizecp, create);
    }
 ...
 */
static int
dt_check_compat_match(struct device_tree_node *node, const char *compat)
{
    list_for_each(prop, node->properties, struct device_tree_property,
                  list_node) {
                                        ...
                                        const char *compat)
{
    ...
    list_for_each(child, parent->children, struct device_tree_node,
                  list_node) {
 ...
 * @return		The found node, or NULL.
 */
struct device_tree_node *dt_find_next_compat_child(struct device_tree_node
                                                   *parent, struct device_tree_node
                                                   *child,
                                                   const char *compat)
{
    int ignore = 0;
    ...
    list_for_each(next, parent->children, struct device_tree_node,
                  list_node) {
        if (ignore) {
            if (child == next)
                ignore = 0;
 ...
 */
struct device_tree_node *dt_find_prop_value(struct device_tree_node
                                            *parent, const char *name,
                                            void *data, size_t size)
{
    /* Check if parent itself has the required property value. */
    list_for_each(prop, parent->properties, struct device_tree_property,
                  list_node) {
...
uint32_t dt_get_phandle(const struct device_tree_node * node)
{
    ...
    dt_find_bin_prop(node, "linux,phandle", (const void **) &phandle,
                     &len);
    if (phandle != NULL && len == sizeof(*phandle))
        return be32_to_cpu(*phandle);
 ...
 */
void
dt_add_bin_prop(struct device_tree_node *node, const char *name,
 ...
 */
void
dt_find_bin_prop(const struct device_tree_node *node, const char *name,
                 const void **data, size_t * size)
{
    *data = NULL;
    *size = 0;
 ...
 */
void
dt_add_string_prop(struct device_tree_node *node, const char *name,
                   const char *str)
{
 ...
 */
void
dt_add_uint32_t_prop(struct device_tree_node *node, const char *name,
                     uint32_t val)
{
    uint32_t *val_ptr = malloc(sizeof(val));
    if (!val_ptr)
        return;
    *val_ptr = cpu_to_be32(val);
 ...
 */
void
dt_add_uint64_t_prop(struct device_tree_node *node, const char *name,
                     uint64_t val)
{
    uint64_t *val_ptr = malloc(sizeof(val));
    if (!val_ptr)
        return;
    *val_ptr = cpu_to_be64(val);
    dt_add_bin_prop(node, name, val_ptr, sizeof(*val_ptr));
}
 ...
 */
void
dt_add_reg_prop(struct device_tree_node *node, uint64_t * addrs,
                uint64_t * sizes, int count, uint32_t addr_cells,
                uint32_t size_cells)
{
    int i;
...
{
    list_for_each(fixup, device_tree_fixups, struct device_tree_fixup,
                  list_node) {
        if (fixup->fixup == NULL)
            return 1;
...
int
dt_set_bin_prop_by_path(struct device_tree *tree, const char *path,
                        void *data, size_t data_size, int create)
{
    ...
    *prop_name++ = '\0';        /* Separate path from the property name. */
    ...
    dt_node =
        dt_find_node_by_path(tree->root, path_copy, NULL, NULL, create);
 ...
 * the node. But dt_add_uint32_t_prop() may leak a bit of memory if you do.
 *
                                                      ...
                                                      *tree)
{
    ...
    // the root.
    dt_add_uint32_t_prop(reserved, "#address-cells", addr);
    dt_add_uint32_t_prop(reserved, "#size-cells", size);
